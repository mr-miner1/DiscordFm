...
module.exports = class DiscordFm extends Plugin {
  startPlugin() {
    ...
    powercord.api.commands.registerCommand({
      ...
      executor: async (args) => {return NowPlaying(args)},
    }),
    powercord.api.commands.registerCommand({
      //todo: error handling
      command: "TrackInfo",
      aliases: ["ti"],
      description: "Get a BUTTLOAD of information on a track (ti)\n[can take upwards of 2mins if you've never heard the track or heard it months ago]",
      usage: "{c} [song - artist or no arguments for info on the track that is currently playing]",
      executor: (args) => {return TrackInfo(args)}
    }),
    powercord.api.commands.registerCommand({
      ...
      executor: async (args) => {return RecentTracks(args)}
    }),
    powercord.api.commands.registerCommand({
      command: "TopTracks",
      aliases: ["tt"],
      ...
      usage: "{c} [optional:pagenumber{'space'}username]",
      executor: async (args) => {return TopTracks(args)},
    }),
    powercord.api.commands.registerCommand({
      command: "TopAlbums",
      aliases: ["tal"],
