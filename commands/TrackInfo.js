...
const TrackInfo = async (args) => {
  const username = powercord.pluginManager
    .get("DiscordFm")
    .settings.get("accountname");
  const layout = powercord.pluginManager
    .get("DiscordFm")
    .settings.get("Layout");
  const bgimage = powercord.pluginManager
    .get("DiscordFm")
    .settings.get("BackgroundImage");
  let time = 0;
  setInterval(() => {
    time = time += 0.1;
  }, 100);
  ...
  const buttonbuilder = () => {
    setTimeout(() => {
      let get_embeds = document.getElementsByClassName("embedFull-1HGV2S");
      for (const i in get_embeds) {
        if (
          get_embeds[i].style.cssText ==
          "border-color: hsl(340, calc(var(--saturation-factor, 1) * 61.1%), 51.6%);"
        ) {
  ...
  if (args == "") {
    //get current track
    let recenttracks = await fetch(
      `https://ws.audioscrobbler.com/2.0/?method=user.getrecenttracks&user=${username}&api_key=b37f440eb014ba5804a5ebf3027447ec&format=json`
    );
    ...
    let track_info = await fetch(
      `https://ws.audioscrobbler.com/2.0/?method=track.getInfo&api_key=b37f440eb014ba5804a5ebf3027447ec&artist=${currenttrack_artist}&username=${username}&track=${currenttrack_name}&format=json`
    );
    ...
    let toptracksapi = await fetch(
      `https://ws.audioscrobbler.com/2.0/?method=user.gettoptracks&user=${username}&api_key=b37f440eb014ba5804a5ebf3027447ec&limit=${1000}&format=json`
    );
    page = 1;
    ...
    for (let i = 0; i < total_tracks; i++) {
      ...
      if (toptrack_number < 1000) {
        ...
        page += 1;
        let stop = false;
        toptracksapi = await fetch(
          `https://ws.audioscrobbler.com/2.0/?method=user.gettoptracks&user=${username}&api_key=b37f440eb014ba5804a5ebf3027447ec&limit=${1000}&page=${page}&format=json`
        );
        toptracksapi = await toptracksapi.json();
    ...
    let geniusapi = await fetch(
      `https://api.genius.com/search?q=${track_name.name}&access_token=JKrnckDfcL0OfjTNXTSd42KU5ccoFScFRc2pz94PVPaf9aaLchyQA3VU4TidiSp2`
    );
    geniusapi = await geniusapi.json();
    genius_title = geniusapi.response.hits[0].result.title;
    genius_url = `/${artist_name.split(" ").join("-")}-${track_name.name
      .split(" ")
      .join("-")}-lyrics`;
    buttonbuilder();
  ...
  //if arguements are given:
    ...
    if (track_name !== undefined && artist_name !== undefined) {
      let track_info = await fetch(
        `https://ws.audioscrobbler.com/2.0/?method=track.getInfo&api_key=b37f440eb014ba5804a5ebf3027447ec&artist=${artist_name}&username=${username}&track=${track_name}&format=json`
      );
      track_info = await track_info.json();
      ...
      page = 1;
      let recenttracks = await fetch(
        `https://ws.audioscrobbler.com/2.0/?method=user.getrecenttracks&user=${username}&api_key=b37f440eb014ba5804a5ebf3027447ec&limit=${200}&format=json`
      );
      recenttracks = await recenttracks.json();
      recenttracks = recenttracks["recenttracks"];
        ...
        //if track is not found on page 1
          ...
          for (let i = 2; i < total_pages; i++) {
            ...
            recenttracks = await fetch(
              `https://ws.audioscrobbler.com/2.0/?method=user.getrecenttracks&user=${username}&api_key=b37f440eb014ba5804a5ebf3027447ec&limit=${200}&page=${page}&format=json`
            );
            recenttracks = await recenttracks.json();
            recenttracks = recenttracks["recenttracks"];
